...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const emailResponse = await resend.emails.send({
      from: "Archie <report@insights.askarchie.ai>",
      to: [email],
    ...
    if ((emailResponse as any)?.error) {
      console.error("Email send failed:", JSON.stringify(emailResponse, null, 2));
      return new Response(
        JSON.stringify({
          success: false,
          error: (emailResponse as any)?.error?.message || "Email send failed",
        }),
        {
          status: 500,
